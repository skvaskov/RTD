...
% load error info
load('rover_sincos_error_functions_T1.5_v0_1.0_to_2.0_degx3_degy3.mat')
...
Z0_range = [[min(A.footprint_vertices(1,:));min(A.footprint_vertices(2,:));0],...
            [max(A.footprint_vertices(1,:));max(A.footprint_vertices(2,:));0]];
...
%% save output
scaling_filename = ['rover_FRS_full_scaling_T',num2str(T,'%0.1f'),...
    '_psiend_',num2str(psi_end_min,'%0.1f'),'_to_',num2str(psi_end_max,'%0.1f'),...
