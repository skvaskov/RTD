    ...
    for w0 = w0_vec
        ...
        for v_des = v_vec
            for psi_end = psiend_vec
                ...
                %restrct yawrate based on difference
                w0_des_min_temp = max(w0_des_min_temp, w0 - diff_w);
                w0_des_max_temp = min(w0_des_max_temp, w0 + diff_w);
                ...
                for w0_des = w0_des_vec
                    ...
                    ev_cos_data(err_idx,:) = z_1(4,:).*cos(z_1(3,:))-v_cos_go;
                    ev_sin_data(err_idx,:) = z_1(4,:).*sin(z_1(3,:))-v_sin_go;
...
v_cos_err_col = abs(ev_cos_data(:)');
v_sin_err_col = abs((ev_sin_data(:)'));

vy_cos_err_col = abs(evy_cos_data(:)');
...
g_v_cos =  fit_bounding_polynomial_from_samples(v_cos_err_col, [T_data_col;psi_ref_data(:)';K_data_col] ,[t;z(3);k],g_degree_x);
g_v_sin =  fit_bounding_polynomial_from_samples(v_sin_err_col, [T_data_col;psi_ref_data(:)';K_data_col] ,[t;z(3);k],g_degree_y);

g_vy_cos = fit_bounding_polynomial_from_samples(vy_cos_err_col,[T_data_col;psi_ref_data(:)';K_data_col] ,[t;z(3);k],g_degree_y);
g_vy_sin = fit_bounding_polynomial_from_samples(vy_sin_err_col,[T_data_col;psi_ref_data(:)';K_data_col] ,[t;z(3);k],g_degree_x);



%% save data

filename = ['rover_sincos_error_functions_T',num2str(T,'%0.1f'),'_v0_',...
            num2str(v0_min,'%0.1f'),'_to_', num2str(v0_max,'%0.1f'),...
            '_degx',num2str(g_degree_x),'_degy',num2str(g_degree_y),'.mat'] ;
save(filename,'A','T','t_f','t','z','k','g_v_cos','g_v_sin','g_vy_cos','g_vy_sin','delta0_min','delta0_max','diff_v','diff_w','psi_end_min','psi_end_max',...
...
g_v_cos_plot = msubs(g_v_cos,[t;z(3);k],[T_data;psi_ref_data(plot_idxs(i),:);K_data_plot]) ;
g_v_sin_plot = msubs(g_v_sin,[t;z(3);k],[T_data;psi_ref_data(plot_idxs(i),:);K_data_plot]) ;

g_vy_cos_plot = msubs(g_vy_cos,[t;z(3);k],[T_data;psi_ref_data(plot_idxs(i),:);K_data_plot]);
g_vy_sin_plot = msubs(g_vy_sin,[t;z(3);k],[T_data;psi_ref_data(plot_idxs(i),:);K_data_plot]);


% plot v error
subplot(2,2,1) ; hold on ;
plot(T_data,abs(ev_cos_data(plot_idxs(i),:)),'*','Color',plot_colors(i,:))
plot(T_data,g_v_cos_plot,'Color',plot_colors(i,:),'LineWidth',1.0) ;

subplot(2,2,3) ; hold on ;
plot(T_data,abs(ev_sin_data(plot_idxs(i),:)),'*','Color',plot_colors(i,:))
plot(T_data,g_v_sin_plot,'Color',plot_colors(i,:),'LineWidth',1.0) ;


% plot vy error
subplot(2,2,4) ; hold on ;
plot(T_data,abs(evy_cos_data(plot_idxs(i),:)),'*','Color',plot_colors(i,:))
plot(T_data,g_vy_cos_plot,'Color',plot_colors(i,:),'LineWidth',1.0) ;

subplot(2,2,2) ; hold on ;
plot(T_data,abs(evy_sin_data(plot_idxs(i),:)),'*','Color',plot_colors(i,:))
